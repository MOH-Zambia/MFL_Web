...
/* @var $searchModel backend\models\ValidationRuleOperatorSearch */
/* @var $dataProvider yii\data\ActiveDataProvider */

$this->title = 'Validation rule operators';
        ...
        <p>
            ...
            if (User::userIsAllowedTo('Manage validation rule operators')) {
                ...
                echo '<button class="btn btn-primary btn-xs" href="#" onclick="$(\'#addNewModal\').modal(); 
                   return false;"><i class="fa fa-plus"></i> Add operator</button>';
            }
            ?>
        </p>
        <?=
                        ...
                        'delete' => function ($url, $model) {
                            if (User::userIsAllowedTo('Remove data element groups')) {
                                return Html::a(
                                            ...
                                            'data-placement' => 'top',
                                            'data' => [
                                                'confirm' => 'Are you sure you want to remove validation rule operator:' . $model->name . '?<br>'
    ...
    <div class="modal-dialog modal-lg">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title">Add new validation rule operator</h5>
                ...
                <div class="row">
                    <div class="col-lg-8">
                        ...
                        $form->field($model, 'name', ['enableAjaxValidation' => true])->textInput(['maxlength' => true, 'placeholder' =>
                            'Name of validation rule operator', 'required' => true,])
                        ?>
                        <?=
                        $form->field($model, 'description')->textarea(['rows' => 4, 'placeholder' =>
                            'Validation rule operator description'])->label("Description ");
                        ?>
                    </div>
                    <div class="col-lg-4">
                        <h4>Instructions</h4>
            ...
            <div class="modal-footer justify-content-between">
                <?= Html::submitButton('Save operator', ['class' => 'btn btn-primary btn-sm']) ?>
                <?php ActiveForm::end() ?>
            </div>
        </div>
        <!-- /.modal-content -->
